<div class="tab-pane fade" id="pills-es" role="tabpanel" aria-labelledby="pills-es-tab">
    <div class="mt-5">
        @php
            $schedules = \App\Models\ScheduleEmail::where('owner_id', \Illuminate\Support\Facades\Auth::id())
                                                    ->with('campaign')
                                                    ->paginate(20);
        ...
        <div class="grid grid-cols-12 gap-6 mt-5">
                            ...
                            <td class="w-40">
                                <div class="flex">
                                    <div class="w-10 h-10 image-fit">
                                        <img alt="{{ $schedule->campaign->name  }}" class="tooltip rounded-full"
                                             src="{{ namevatar($schedule->campaign->name) }}"
                                             title="{{ $schedule->campaign->name }}">
                                    </div>
                                </div>
                            </td>
                            <td>
                                <a href="javascript:;" class="font-medium whitespace-no-wrap tooltip inline-block"
                                   title="{{ $schedule->campaign->name  }}">{{$schedule->campaign->name }}</a>
                                <div class="text-gray-600 text-xs whitespace-no-wrap"
                                     data-theme="light">{!! Str::limit($schedule->campaign->description, 150) !!}</div>
                            </td>
                            ...
                            <td class="w-40">
                                <div class="flex items-center justify-center {{ $schedule->status == 'SENT' ? 'text-theme-9' : 'text-theme-6' }}">
                                    <i data-feather="check-square"
                                       class="w-4 h-4 mr-2"></i> {{ $schedule->status == 'SENT' ? 'Sent' : 'Pending' }}
                                </div>
                            </td>
                            ...
                            <td class="table-report__action w-56">
                                <div class="flex justify-center items-center">
                                    ...
                                    <a class="flex items-center mr-3 tooltip" title="@translate(Edit)"
                                       href="{{ route('campaign.schedule.email.edit', $schedule->id) }}">
                                        <i data-feather="check-square" class="w-4 h-4 mr-1"></i>
                                    </a>
                                    <a class="flex items-center text-theme-6 tooltip"
                                       href="{{ route('campaign.schedule.email.delete', $schedule->id) }}"
            ...
            <div class="intro-y col-span-12 text-center">
                <div class="md:block mx-auto text-gray-600">Showing {{ $schedules->firstItem() ?? '0' }}
                    to {{ $schedules->lastItem() ?? '0' }} of {{ $schedules->total() }} entries
                </div>
            </div>
            <!-- END: Data List -->
            <!-- BEGIN: Pagination -->
