...
@section('subhead')
    <title>{{ $autoresponder->name }}</title>
@endsection

@section('subcontent')
<div class="flex items-center mt-8">
        <h2 class="intro-y text-lg font-medium mr-auto">{{ $autoresponder->name }}</h2>
    </div>
    <!-- BEGIN: Wizard Layout -->
    <div class="intro-y box py-10 sm:py-20 mt-5">
        <div class="wizard flex lg:flex-row justify-center px-5 sm:px-20">
            <div class="intro-x lg:text-center flex items-center lg:block flex-1 z-10">
                ...
                <div class="lg:w-32 font-medium text-base lg:mt-3 ml-3 lg:mx-auto">{{ $autoresponder->name }}</div>
            </div>
            <div class="intro-x lg:text-center flex items-center mt-5 lg:mt-0 lg:block flex-1 z-10">
                <button class="w-10 h-10 rounded-full button text-gray-600 bg-gray-200 dark:bg-dark-1">2</button>
                <div class="lg:w-32 text-base lg:mt-3 ml-3 lg:mx-auto text-gray-700 dark:text-gray-600">@translate(Update Template)</div>
            </div>
        ...
        <div class="px-5 sm:px-20 mt-10 pt-10 border-t border-gray-200 dark:border-dark-5">
            <!-- BEGIN: Form Layout -->
            <form action="{{ route('autoresponder.builder.edit', $autoresponder->id) }}" method="GET">
                <div class="intro-y box p-5">
                        ...
                        <select class="w-full form-select sm:w-1/2" name="campaign_id" required>
                            @forelse (allCampaigns('email') as $campaign)
                                <option value="{{ $campaign->id }}" 
                                        class="normal-case" {{ $autoresponder->campaign_id == $campaign->id ? 'selected' : null }}>
                    ...
                    <div class="mt-3">
                        ...
                        <div class="mt-2">
                            <input type="checkbox" value="1" class="input input--switch border" name="status" {{ $autoresponder->status == 1 ? 'checked' : null }}>
                        </div>
                    </div>
