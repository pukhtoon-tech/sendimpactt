...
var _menuSelector = '.bal-context-menu';
...
function _menuEditor(element, event) {
    ...
    $('.bal-content-main .highlight').removeClass('highlight');
...
$(document).on('keyup','.context-menu-hyperlink-input',function(){
  _spanId = $('.bal-context-menu').attr('data-span-id');
  _span   = $('#span_'+_spanId);
...
$(document).on('click', '.bal-context-menu-item', function (e) {
    e.preventDefault();

    _menuType = $(this).attr('data-menu-type');
    _spanId=$(this).parents('.bal-context-menu').attr('data-span-id');
    _span=$('#span_'+_spanId);
