  ...
  <div class="flex items-center mt-8">
    ...
    <div class="intro-y box py-10 sm:py-20 mt-5">
        ...
        <div class="px-5 sm:px-20 mt-10 pt-10 border-t border-gray-200 dark:border-dark-5">
            <!-- BEGIN: Form Layout -->
            <form action="{{ route('campaign.emails.update', $edit_campaign->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="intro-y box p-5">
                            ...
                            @forelse (getSmtpServerWiseList() as $provider => $email_provider)
                                <optgroup label="{{ Str::upper($provider) }} SERVER('s)">
                                    @forelse ($email_provider as $provider)
                                        <option value="{{ $provider->id }}" {{ $edit_campaign->smtp_server_id == $provider->id ? 'selected' : null }} class="normal-case">{{ Str::camel($provider->provider_name) }}</option>
                                    @empty
                    ...
                    <div class="mt-3">
                        ...
                        <a href="{{ asset($edit_campaign->campaign_attachment->attachment) }}">
                            <img src="{{ asset('pdf.png') }}" class="w-20" alt="{{ __('Attachment') }}">
                        </a>

                        <div class="mt-3">
                            <a href="{{ route('campaign.remove.pdf', $edit_campaign->id) }}" class="button w-24 bg-theme-1 text-white">{{ __('Remove attachment') }}</a>
                        </div>
                    </div>
                    @endif
...
<input type="hidden" id="campaign_emails_edit_url" value="{{ route('campaign.contacts.emails.edit', $edit_campaign->id) }}">
