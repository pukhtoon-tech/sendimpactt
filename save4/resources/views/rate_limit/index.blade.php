                ...
                <div class="box">
                    <div class="flex flex-col lg:flex-row items-center p-5 border-b border-gray-200 dark:border-dark-5">
                        <div class="w-24 h-24 lg:w-12 lg:h-12 image-fit lg:mr-1">
                            <img alt="{{ $limit->name }}" class="rounded-full tooltip" title="{{ $limit->name }}" src="{{ namevatar($limit->name) }}">
                        </div>
                        ...
                        <div class="flex -ml-2 lg:ml-0 lg:justify-end mt-3 lg:mt-0">
                            @if ($limit->limit != null)
                                <a href="{{ route('limit.manage', $limit->limit->id) }}" 
                                    class="button button--sm text-gray-700 border border-gray-300 dark:border-dark-5 dark:text-gray-300 m-2">
                                    @translate(Manage Limit)
                                </a>
                            @else
                            <a href="javascript:;" 
                                class="button button--sm text-white bg-theme-6 border border-gray-300 dark:border-dark-5 dark:text-white m-2">
                            ...
                            @can('Admin')
                                ...
                                <a href="{{ route('limit.destroy', $limit->id) }}" 
                                    class="button button--sm text-white bg-theme-7 border border-gray-300 dark:border-dark-5 dark:text-white m-2">
                                    @translate(Remove)
                                </a>

                                @if (Auth::id() != $limit->limit->owner_id)
                                <a href="{{ route('login.as.customer', $limit->limit->owner_id) }}" 
                                    class="button button--sm text-white bg-theme-1 border border-gray-300 dark:border-dark-5 dark:text-white m-2">
                                    @translate(Login)
                                </a>
                                @endif
                        ...
                        <div class="w-full lg:w-1/2 mb-4 lg:mb-0 mr-auto">
                            <div>
                                ...
                                <div class="w-full h-4 bg-gray-400 dark:bg-dark-1 rounded">
                                    <div class="{{ emailLimitProgressBar($limit->limit->owner_id) }} h-full rounded text-center text-xs text-white">{{ round(emailLimitCheckPercentage($limit->limit->owner_id)) }}%</div>
                                </div>
                            </div>
                            ...
                            <div class="my-3">
                                ...
                                <div class="w-full h-4 bg-gray-400 dark:bg-dark-1 rounded">
                                    <div class="{{ smsLimitProgressBar($limit->limit->owner_id) }} h-full rounded text-center text-xs text-white">{{ round(smsLimitCheckPercentage($limit->limit->owner_id)) }}%</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-wrap lg:flex-no-wrap items-center justify-center p-5">
                        <div class="w-full lg:w-1/2 mb-4 lg:mb-0 mr-auto">
                            <div>
                                <label for="">@translate(Emails)</label>
                                <div class="w-full h-4 bg-gray-400 dark:bg-dark-1 rounded">
                                    <div class="{{ emailLimitProgressBar($limit->id) }} h-full rounded text-center text-xs text-white">0%</div>
                                </div>
                            </div>
                            ...
                            <div class="my-3">
                                <label for="">@translate(SMS)</label>
                                <div class="w-full h-4 bg-gray-400 dark:bg-dark-1 rounded">
                                    <div class="{{ smsLimitProgressBar($limit->id) }} h-full rounded text-center text-xs text-white">0%</div>
                                </div>
                            </div>
                        </div>
                    </div>
